...
import { clearWindowJumpList, setMacOSRecentDocuments, setWindowJumpList } from './history'
...
const register = (): void => {
  ...
  ipcMain.on(IpcEvents.EV_GET_SUBTITLE, (e, subtitlePath: string) => {
    if (fs.existsSync(subtitlePath + '.srt')) {
      ...
      e.reply(IpcEvents.EV_GET_SUBTITLE_REPLY, { subtitle })
    }
  })
  ...
  ipcMain.handle(IpcEvents.EV_CLEAR_PLAYLIST, () => {
    clearWindowJumpList()
    store.clear()
    return
  })
