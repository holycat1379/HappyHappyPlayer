      ...
      <Transition>
        <p v-show="enShow">
          <span
            ...
            @mouseover="throttle(() => highlightWord(index), 1000)"
            @mouseout="throttle(() => highlightWord(null), 1000)"
          >
            {{ word }}
          </span>
        </p>
...
const highlightWordThrottled = (index: number) => {
  return throttle(() => highlightWord(index), 1000)
}

function highlightWord(index: number) {
  highlightedWordIndex.value = index
  if (words.value[index]) {
    // fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${words.value[index]}`).then(
    //   (response) => {
