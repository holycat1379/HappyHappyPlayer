<template>
  <div class="subtitle-item pt-10">
    ...
    <div class="text-center ch-box" >{{ subtitle.ch }}</div>
  </div>
</template>
...
onBeforeMount(() => {
  ...
  // 当 鼠标悬浮在 ch-box 上时，显示当前字幕的中文翻译，否则显示灰色背景
  const chBox = document.querySelector('.ch-box')
  chBox?.addEventListener('mouseenter', () => {
    chBox.classList.remove('bg-gray-200')
  })
  chBox?.addEventListener('mouseleave', () => {
    chBox.classList.add('bg-gray-200')
  })
...
function youtubeModeSrtHandle(): Line[] {
  ...
  for (let i = 0; i < props.srtList.length; i += 2) {
