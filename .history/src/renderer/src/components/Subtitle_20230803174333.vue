...
onBeforeMount(() => {
  handledSrtList.value = youtubeModeSrtHandle()
})

// 将srtList的第1个和第2个合并为一个字幕，第3个和第4个合并为一个字幕，以此类推
function youtubeModeSrtHandle(): Line[] {
  const mergedSrtList: Line[] = []
  for (let i = 0; i < handledSrtList.value.length; i += 2) {
  ...
  () => {
    ...
    for (let i = 0; i < handledSrtList.value.length; i++) {
      ...
      if (
        ...
        subtitle.value = currentSrtItem.text
        break
      }
    }
  }
...
//     handledSrtList.value.forEach((item, index) => {
//       // 如果是最后一个 则下一个开始时间为0 也就是视频结束时间
//       let nextStartSeconds = handledSrtList.value[index + 1] ? handledSrtList.value[index + 1].startSeconds : 0
//
//       // 如果两者相等说明存在同样两个的时间 就需要判断下一个的下一个时间
//       if (nextStartSeconds === item.startSeconds && handledSrtList.value[index + 2]) {
//         nextStartSeconds = handledSrtList.value[index + 2].startSeconds
//       }
//
