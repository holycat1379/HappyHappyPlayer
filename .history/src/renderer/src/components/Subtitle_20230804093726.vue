<template>
  <div class="subtitle-item pt-10">
    <div class="ch-box h-20 text-2xl w-auto bg-dark-50 text-center" @mouseup="getSelectedText">
      <Transition>
        <p v-show="enShow">
          <span v-for="(word, index) in words" :key="index" @mouseover="highlightWord(word)" @mouseout="highlightWord(null)">
            {{ word }}<span v-if="highlightedWord === word" class="highlight"> </span>
          </span>
        </p>
      </Transition>
    </div>
