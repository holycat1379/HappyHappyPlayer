<template>
  <div class="subtitle-item pt-10">
    <p v-show="enShow" class="text-2xl text-center">{{ subtitle.text }}</p>
    <div
...
console.log(props.player?.instance.isPaused)
watch(
  () => props.player?.instance.isPaused,
  () => {
    console.log(123)
    if (props.player?.isPaused) {
      enShow.value = true
      chShow.value = true
    } else {
      enShow.value = false
