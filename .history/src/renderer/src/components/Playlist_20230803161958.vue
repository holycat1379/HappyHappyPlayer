...
const handleClear = (): void => {
  window.electron.ipcRenderer.invoke(IpcEvents.EV_CLEAR_PLAYLIST)
...
<template>
  <n-drawer v-model:show="expand" :width="300" placement="right">
    <n-drawer-content closable>
      <template #header>
        历史记录
        <n-dropdown trigger="hover" :options="options" @select="handleClear">
    ...
    <div class="absolute bottom-0 w-full border-true-gray-300 h-8 border border-solid" :border="false">清空历史记录</div>
  </n-drawer>
</template>
