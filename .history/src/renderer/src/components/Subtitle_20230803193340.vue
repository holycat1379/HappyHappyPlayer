<template>
  <div class="subtitle-item pt-10">
    <div class="ch-box h-20 text-2xl w-auto bg-dark-50 text-center">
      <Transition>
        <p v-show="enShow" v-html="highlightWords(subtitle.text)"></p>
      </Transition>
    </div>
    <div
...
function highlightWords(text: string) {
  ...
  return words
    .map((word) => {
      return `<span v-bind:class="{ 'highlight': hovering }" v-on:mouseover="hovering = true" v-on:mouseout="hovering = false">${word}</span>`
    })
    .join(' ')
}
